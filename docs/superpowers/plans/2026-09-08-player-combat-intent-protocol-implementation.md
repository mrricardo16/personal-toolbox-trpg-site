...
MeleeAttack_OtherPlayersCharacter_ReturnsActorNotOwnedBeforeDiceOrMutation
MeleeAttack_OwnButNonCurrentInvestigator_ReturnsNotCurrentActorBeforeDiceOrMutation
MeleeAttack_IneligibleTarget_ReturnsTargetNotEligibleBeforeDiceOrMutation
...
For stale input, either actor rejection, or target rejection, assert zero calls to Begin/Resolve/Damage and unchanged fake dice/notifier counters. Assert no `ExchangeId` is generated by observing unchanged canonical pending/history/revision. The other-player actor case returns `ActorNotOwned`; an owned but non-current investigator returns `NotCurrentActor`.
...
if (combat is null || !combat.Active)
{
    return PlayerCombatIntentResult.Failure(PlayerCombatIntentErrorCode.CombatInactive, snapshot.Revision);
}

var actor = combat.Participants.SingleOrDefault(
...
if (!actor.Active
    || !actor.Current
    || combat.CurrentActorParticipantId != actor.ParticipantId)
{
    return PlayerCombatIntentResult.Failure(PlayerCombatIntentErrorCode.NotCurrentActor, snapshot.Revision);
}

var actions = combat.ViewerActions;
...
var pending = combat.ViewerActions?.PendingResponse;
if (pending is null)
{
    return PlayerCombatIntentResult.Failure(
...
Expected GREEN: all selected tests pass; only the exact owner can resolve; stale/wrong/unavailable inputs roll and mutate zero times; optional Damage uses the post-Resolve revision. A viewer who sees generic pending Combat but no `PendingResponse` receives `DefenderNotOwned` without receiving an exact exchange ID, available responses, defender owner identity, NPC policy, or response allowance.
...
**Interfaces:**
 ...
 - Consumes: `PlayerPassIntent`, viewer-safe Combat participants plus projected `ActorCharacterId`/`CanPass`, and canonical `PassCombatTurnAsync`.
...
Pass_OtherPlayersCharacter_ReturnsActorNotOwnedBeforeMutation
Pass_OwnButNonCurrentInvestigator_ReturnsNotCurrentActorBeforeMutation
...
- [ ] **Step 7: Run the workflow-derived authoritative 37 Single Player regressions and the legacy alias separately.**

```powershell
$regressions = @(
  'build/test-security-hardening.js',
  'build/test-save-ui.js',
  'build/test-coc-outcomes.js',
  'build/test-situation-ui.js',
  'build/test-ai-json-repair.js',
  'build/test-v150-experience.js',
  'build/test-v151-investigation-stability.js',
  'build/test-v152-long-session.js',
  'build/test-v153-clue-routes.js',
  'build/test-v154-protocol-routing.js',
  'build/test-v154-npc-materialization.js',
  'build/test-v154-location-alias.js',
  'build/test-v154-empty-response-retry.js',
  'build/test-v154-operation-id-alias.js',
  'build/test-v155-location-transition-alias.js',
  'build/test-v156-player-assertion-guard.js',
  'build/test-v157-case-integrity.js',
  'build/test-v157-canonical-assertion-state.js',
  'build/test-v158-api-response-resilience.js',
  'build/test-v159-progress-semantics.js',
  'build/test-v1510-authored-threat-clock.js',
  'build/test-v1511-npc-knowledge-boundary.js',
  'build/test-v1512-ending-resolution-gate.js',
  'build/test-v1513-full-case-e2e.js',
  'build/test-v160-coc-resolution-engine.js',
  'build/test-v160-coc-resolution-diagnostics.js',
  'build/test-v160-transport-abort-classification.js',
  ...
  'build/test-v169-combat-damage.js',
  'build/test-v1610-firearms-impaling.js'
)
if ($regressions.Count -ne 37) { throw "Expected exactly 37 workflow-authoritative offline regressions, got $($regressions.Count)" }
foreach ($test in $regressions) {
  node $test
  if ($LASTEXITCODE -ne 0) { throw "Authoritative offline regression failed: $test" }
}
node build/test-protocol-stability.js
if ($LASTEXITCODE -ne 0) { throw 'legacy test-protocol-stability alias failed' }
```

Expected: the explicit workflow-derived offline allowlist reports 37/37 PASS and the separate legacy alias reports PASS. Do not use a broad `build/test-*.js` filesystem glob as the authoritative suite selector. Do not run `build/test-real-api-v1513.js` or `build/test-real-api-v152.js`: credentialed real API testing is excluded and not required.
