...
Phase 2H deliberately rejects unowned NPC actors even for the Host. Its current application coordinator has only `IGameCoordinator` and `IInternalCombatResolutionCoordinator` dependencies and must not become the owner of NPC behavior.
...
The initial policy is uniform and deterministic:
...
Pass does not end Combat or invent a target. It reuses `PassCombatTurn`, preserving action counts, stable active scanning, round wrap, and dying scheduling. The driver never calls `EndCombat`; existing damage/turn rules remain responsible for canonical termination.
...
Phase 2I therefore needs a narrow server-internal application execution seam, conceptually:

```text
IInternalNpcCombatTurnExecutor
├─ BeginNpcOpposedExchange(RoomId, ExpectedRevision, NpcParticipantId, TargetParticipantId)
└─ PassNpcCombatTurn(RoomId, ExpectedRevision, NpcParticipantId)
```
...
Because current Begin/Pass core methods contain player/Host authorization inline, future implementation must separate that authorization from a shared private canonical validation/mutation helper. The existing player/Host entry point performs its current authorization before calling the helper; the new NPC entry point performs exact unowned-current-opponent authorization before calling the same helper. The design does not permit bypassing the existing method with reflection, passing a sentinel `Guid`, or weakening the canonical validations.
...
`PlayerCombatIntentCoordinator` remains the human-intent coordinator. It passes the latest committed state into continuation after successful paths and obtains a final viewer projection afterward. `GameCoordinator` remains authoritative for mutation and mechanics, not policy selection.
...
The exact record is never returned by HTTP, projected, serialized, or registered as a general query service. `ResolveExactPendingDamageAsync` should return the Resolve state when there is no pending damage and the Damage result state when damage commits. Pass should retain `PassCombatTurnResult.State`. Melee/Respond should update `lastCommittedState` after every success.
...
Each existing changed transition publishes exactly one viewer-specific `GameSnapshot` after its commit. The continuation layer does not publish.
...
- inactive, defeated, same-side, non-investigator, malformed, or hidden invalid participants are never selected.
- NPC current with no legal target: canonical Pass once, inspect returned state.
...
3. **How does NPC choose Attack vs Pass?** Attack when at least one legal melee target exists; otherwise canonical Pass.
...
1. Add a dedicated `ICombatContinuationCoordinator` and small deterministic `NpcCombatTurnDriver` selector, outside both `PlayerCombatIntentCoordinator` and `GameCoordinator` rule ownership.
2. Add narrow internal NPC Begin/Pass execution commands that carry exact NPC participant identity and revision but no PlayerId, Host identity, public flag, or caller-selected trust.
3. Select the first active opposing investigator in stable canonical order; Attack when one exists, otherwise canonical Pass.
...
This design is pending review. It does not authorize an implementation plan or implementation work.
